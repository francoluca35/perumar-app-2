...
export default function CajaRetiro() {
  ...
  const fetchInforme = async () => {
    ...
    ingresos.forEach((item) => {
      const fecha = format(
        new Date(item.timestamp?.toDate?.() || item.fecha),
        "yyyy-MM-dd"
      );
      if (!porFecha[fecha]) porFecha[fecha] = { ingresos: [], retiros: [] };
      porFecha[fecha].ingresos.push(item);
    });
    ...
    retiros.forEach((item) => {
      ...
      if (!porFecha[fecha]) porFecha[fecha] = { ingresos: [], retiros: [] };
      porFecha[fecha].retiros.push(item);
    });

    const informeFinal = Object.entries(porFecha).map(([fecha, data]) => {
      const ingresoTotal = data.ingresos.reduce(
        (acc, item) => acc + (item.ingresoTotal || 0),
        0
      );
      const retirosTotal = data.retiros.reduce(
      ...
      return {
        ...
        retiros: data.retiros.map((r) => ({
          ...
          motivo: r.motivo,
          hora: format(
            new Date(r.timestamp?.toDate?.() || r.timestamp),
            "HH:mm"
          ),
        })),
      };
  ...
  const descargarExcel = async (tipo) => {
    ...
    try {
      ...
      // 🔥 Borrar datos si tipo === 'todo'
      if (tipo === "todo") {
        await fetch("/api/informe/borrar-todo", { method: "DELETE" });
      }

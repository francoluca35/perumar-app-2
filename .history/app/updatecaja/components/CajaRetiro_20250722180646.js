...
export default function CajaRetiro() {
  ...
  const fetchInforme = async () => {
    const snapshot = await getDocs(collection(db, "ingresosDiarios"));

    const ingresosRaw = snapshot.docs.map((doc) => doc.data());

    const ingresosPorFecha = {};

    ingresosRaw.forEach((item) => {
      // Asegurarse de tener timestamp (usá el campo real que tengas)
      const fechaTimestamp = item.timestamp || item.createdAt || item.fecha;

      // Convertir a fecha yyyy-mm-dd
      const fecha =
        typeof fechaTimestamp === "string"
          ? fechaTimestamp.slice(0, 10)
          : format(
              new Date(fechaTimestamp.toDate?.() || fechaTimestamp),
              "yyyy-MM-dd",
              { locale: es }
            );

      if (!ingresosPorFecha[fecha]) ingresosPorFecha[fecha] = [];
      ingresosPorFecha[fecha].push(item);
    });

    const informeFinal = Object.entries(ingresosPorFecha).map(
      ([fecha, registros]) => {
        const ingresoTotal = registros.reduce(
          (acc, item) => acc + (item.ingresoTotal || 0),
          0
        );
        return {
          fecha,
    ...
    const ordenado = informeFinal.sort(
              ...
              <div className="text-sm">
                <p>
                  ...
                  <span className="text-green-400">
                    ${(item.ingresoTotal || 0).toLocaleString()}
                  </span>
                </p>
                <p>
                  Retiros:{" "}
                  <span className="text-red-400">
                    -${(item.retirosTotal || 0).toLocaleString()}
                  </span>
                </p>
                <p>
                  Neto:{" "}
                  ...
                  <>
                    <p>
                      ...
                      <span className="text-blue-400">
                        ${(item.cierreCaja || 0).toLocaleString()}
                      </span>
                    </p>
                    {item.horaCierre && (
              ...
              {mostrarDetalles[item.fecha] && item.retiros?.length > 0 && (
                <ul className="mt-2 text-xs text-gray-300 list-disc list-inside">
                  {item.retiros.map((r, j) => (
                    <li key={j}>
                      {r.hora} - ${(r.monto || 0).toLocaleString()} - {r.motivo}
                    </li>
                  ))}
                </ul>
              )}

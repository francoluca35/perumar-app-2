...
export default function RestauranteForm() {
  ...
  const enviarPedidoFinal = async () => {
    ...
    const payload = {
      ...
      modo: "entregalocal", // <- esta línea está bien así, NO uses `modo` suelto
      estado: "en curso",
      ...
      if (res.ok) {
        ...
        await fetch("/api/print/envios", {
          ...
          body: JSON.stringify({
            ...
            fecha,
            metodoPago: pago,
            modo: "entregalocal", // 👈🏼 ESTA ES LA LÍNEA CORRECTA
          }),
        });
        // 🔽 Descarga PDF automáticamente
        const pdfBlob = await fetch("/api/pdf/envio", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            nombre,
            ...
            fecha,
            metodoPago: pago,
            modo: "retiro", // 👈 importante para que diga 'RETIRO LOCAL'
          }),
        }).then((res) => res.blob());

        // ⬇️ Crear link y forzar descarga
        const url = window.URL.createObjectURL(pdfBlob);
        const a = document.createElement("a");
        a.href = url;
        a.download = `ticket-retiro-${nombre}.pdf`;
        a.click();

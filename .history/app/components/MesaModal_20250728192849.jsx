  ...
  const imprimirTicket = async (productos, mesa, orden, hora, fecha) => {
    ...
    const enviarAImpresora = async (items, ip) => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("Fallo al imprimir");
        return await res.json();
      } catch (err) {
        console.warn(`❌ Error al imprimir en ${ip}:`, err.message);
        return null;
      }
    };

    const [resParrilla, resCocina] = await Promise.all([
      enviarAImpresora(parrilla, "192.168.0.101"),
      enviarAImpresora(cocina, "192.168.0.100"),
    ]);

    const preview = resParrilla?.preview || resCocina?.preview;
    if (!preview) return;
    ...
    try {
      ...
      Swal.fire({
        ...
        text: "Ticket impreso y descargado correctamente.",
        timer: 2000,
      });
    } catch (err) {
      ...
      Swal.fire({
        icon: "warning",
        title: "Impresión exitosa, pero no se pudo descargar el PDF",
      });
    }
  };

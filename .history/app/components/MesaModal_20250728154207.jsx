...
import useGuardarDatosCliente from "../hooks/useGuardarDatosClientes";

export default function ModalMesa({ mesa, onClose, refetch }) {
  const { productos } = useProductos();
  const guardarDatosCliente = useGuardarDatosCliente();
  ...
  const [bloquearMesero, setBloquearMesero] = useState(false);

  useEffect(() => {
    if (mesa.estado === "ocupado") {
      setHistorial(mesa.productos || []);
      setMetodoPago(mesa.metodoPago || "");
      setMesero(mesa.mesero || "");
      setBloquearMesero(true); // ✅ bloqueamos edición
    }
  }, [mesa]);
  ...
  useEffect(() => {
    ...
    if (mesa.estado === "libre") {
      ...
      setPedidoActual([]);
      setMesero("");
      setBloquearMesero(false); // ✅ volvemos a habilitar input
    }
  }, [mesa]);
  ...
  const imprimirTicket = async (productos, mesa, orden, hora, fecha) => {
    ...
    const enviarAImpresora = async (items, ip) => {
      if (items.length === 0) return null; // ⬅ importante devolver null explícitamente
      ...
      try {
        ...
        return json; // ✅ ahora devuelve la respuesta, que incluye el link del PDF
      } catch (err) {
    ...
    const respuestas = [];
    mesa.empleado = mesero;
    respuestas.push(await enviarAImpresora(parrilla, "192.168.0.101"));
    respuestas.push(await enviarAImpresora(cocina, "192.168.0.100"));

    const preview = respuestas.find((res) => res?.preview);
    if (preview) {
      try {
        ...
        a.href = url;
        a.download = `ticket-${mesa.tipoMesa}-${mesa.numero}.pdf`;
        ...
        await Swal.fire({
          icon: "success",
          title: "Pedido impreso y descargado",
          text: "El ticket fue enviado a la impresora y se descargó correctamente.",
          timer: 2000,
        });
      } catch (err) {
        ...
        Swal.fire({
          icon: "warning",
          title: "Impresión OK, pero no se pudo descargar el PDF",
        });
      }
    }
  };
  ...
  const enviarPedido = async () => {
    ...
    try {
      ...
      // ✅ Guardar datos en Firebase para estadísticas
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center px-4">
      <div className="bg-[#1a1a1a] border border-white/10 w-full max-w-5xl rounded-2xl p-6 shadow-2xl text-white max-h-screen overflow-y-auto space-y-6">
        ...
        <input
          ...
          className="w-full bg-white/10 border border-white/20 text-white placeholder-white/50 py-2 px-4 rounded-xl mb-2"
          disabled={bloquearMesero}
        />
